...
class LoginController < ApplicationController
  ...
  def complete_openid_auth
    # FIXME - url_for some action is not necessarily the current URL.
    current_url = url_for(:action => 'complete_openid_auth', :only_path => false)
  ...
  def open_id_authentication identifier
    ...
    return_to = url_for :action => 'complete_openid_auth', :only_path => false
  ...
  def consumer
    if @consumer.nil?
      ...
      @consumer = OpenID::Consumer.new(session, nil) #store)
    end
    return @consumer
  end

  def password_authentication(login, pass)
